...
def display_and_select_slices_in_single_window(file_path):
    """
    ...
    def submit_selection():
        try:
            for i, (path, shape) in enumerate(datasets_3d):
                ...
                if not slice_value.isdigit() or not (0 <= int(slice_value) < shape[int(dimensions[i].get())]):
                    raise ValueError(f"Slice invalide pour {path}. Entrez une valeur entre 0 et {shape[int(dimensions[i].get())] - 1}.")
                
                # Récupérer la dimension
                dimension_value = dimensions[i].get()
                if dimension_value not in ("0", "1", "2"):
                    raise ValueError(f"Dimension invalide pour {path}. Choisissez 0, 1 ou 2.")
                
                selections[path] = {"slice": int(slice_value), "dimension": int(dimension_value)}
    ...
    try:
        with h5py.File(file_path, "r") as f:
            ...
            if not datasets_3d:
                messagebox.showinfo("Informations", "Aucune image 3D trouvée dans le fichier.")
                return {}
            ...
            # Titre général
            tk.Label(root, text="Sélectionnez une slice et une dimension pour chaque image 3D", font=("Arial", 14)).grid(row=0, column=0, columnspan=4, pady=10)

            # Tableau des informations
            tk.Label(root, text="Chemin de l'image", font=("Arial", 12, "bold")).grid(row=1, column=0, padx=5, pady=5, sticky="w")
            tk.Label(root, text="Dimensions", font=("Arial", 12, "bold")).grid(row=1, column=1, padx=5, pady=5, sticky="w")
            tk.Label(root, text="Slice sélectionnée", font=("Arial", 12, "bold")).grid(row=1, column=2, padx=5, pady=5, sticky="w")
            tk.Label(root, text="Dimension", font=("Arial", 12, "bold")).grid(row=1, column=3, padx=5, pady=5, sticky="w")
            ...
            selections = {}
            for i, (path, shape) in enumerate(datasets_3d):
                tk.Label(root, text=path, wraplength=300).grid(row=i + 2, column=0, padx=5, pady=5, sticky="w")
                tk.Label(root, text=str(shape)).grid(row=i + 2, column=1, padx=5, pady=5, sticky="w")
                ...
                dimension_menu = ttk.Combobox(root, textvariable=dimension_var, values=["0", "1", "2"], state="readonly", width=5)
                dimension_menu.grid(row=i + 2, column=3, padx=5, pady=5)
                dimensions.append(dimension_var)

            # Bouton de validation
            submit_button = ttk.Button(root, text="Valider", command=submit_selection)
            submit_button.grid(row=len(datasets_3d) + 2, column=0, columnspan=4, pady=10)
...
def read_hdf5(paths):
    ...
    for dataset_key, images in dataset_layers.items():
        try:
            ...
            print(f"Erreur lors de l'empilement des images pour la clé {dataset_key}: {e}")
            continue
